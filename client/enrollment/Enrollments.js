...
import { makeStyles, GridList, GridListTileBar, GridListTile } from "@material-ui/core"
...
export default function Enrollments(props) {
    ...
    return (
        <div>
            <GridList cellHeight={120} className={classes.gridList} col={4}>
                {props.enrollments.map((course, i) => (
                    <GridListTile key={i} className={classes.tile}>
                        <Link to={"/learn/"+course._id}>
                            <img classname={classes.image} src={'/api/courses/photp/'+course.course._id} alt={course.course.name} alt={course.course.name} />
                        </Link>
                        <GridListTileBar className={classes.tileBar} 
                            ...
                            actionIcon={<div className={classes.action}>
                                ...
                                : (<InProgressIcon className={classes.progress} />) } </div>} />
                    </GridListTile>
                ))}
            </GridList>
        </div>
